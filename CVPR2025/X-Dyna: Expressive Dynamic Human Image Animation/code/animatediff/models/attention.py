        ...
        self.transformer_blocks = nn.ModuleList(
            [
                BasicTransformerBlock(
                    ...
                    use_reference_image_cfg = use_reference_image_cfg,
                    cfg_ratio = cfg_ratio,
                )
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        self.use_reference_image_cfg = use_reference_image_cfg
        ...
        if unet_use_cross_frame_attention == "first_cross":
            ...
            self.attn1 = MixedCausalAttention2D(
                use_reference_cfg=self.use_reference_image_cfg,
...
class SparseCausalAttention2D(Attention):
...
# mixed attention 
class MixedCausalAttention2D(Attention):
    """
    MixedCausalAttention: compute attention on reference image (to_q_ff, to_out_ff) and self-attention on hidden_states (to_q, to_k, to_v, to_out)
    # TODO: q, k, v
    """
    def __init__(self, use_reference_cfg, cfg_ratio, *args, **kwargs):
        ...
        self.use_reference_cfg = use_reference_cfg
        self.cfg_ratio = cfg_ratio
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, video_length=None, use_temp=None):
        ...
        query_ff = self.head_to_batch_dim(query_ff)
        ...
        key_ff = self.head_to_batch_dim(key_ff)
        value_ff = self.head_to_batch_dim(value_ff)
        ...
        # linear proj
        hidden_states = self.to_out[0](hidden_states)
        # NOTE: first_frame_hidden_states scale can be tuned if necessary
...
class IPAdapterAttention(Attention):
    """
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        if encoder_hidden_states.shape[1] == 93: # ip_adapter 99 =  text 77 + image 16
        ...
        if self.added_kv_proj_dim is not None:
            ...
            key = self.head_to_batch_dim(key)
            value = self.head_to_batch_dim(value)
            encoder_hidden_states_key_proj = self.head_to_batch_dim(encoder_hidden_states_key_proj)
            encoder_hidden_states_value_proj = self.head_to_batch_dim(encoder_hidden_states_value_proj)
        ...
        key_ip = self.head_to_batch_dim(key_ip)
        value_ip = self.head_to_batch_dim(value_ip)
        ...
        # mixed attention outputs 
        # NOTE: image embedding scale can be tuned
