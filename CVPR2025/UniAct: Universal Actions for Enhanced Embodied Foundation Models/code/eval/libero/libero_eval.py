...
a_mean, a_std = np.array(AIRDATA_CONFIG['libero-1-rgb']['action_statics']['mean']), np.array(AIRDATA_CONFIG['libero-1-rgb']['action_statics']['std'])
...
class LIBEROEval():
       ...
       def _rollout(self, task_suite, policy, task_id: int=0, img_goal=False, use_ac=True, code_type="gumbel_hard", k=0., temperature=1.0):
              ...
              for t in tqdm(range(self.eval_horizon), desc=f'{lang}'):
                     ...
                     image_np = np.asarray(BASIC_TRANSFORM(image))[None, ...]
                     image_tensor = self.r18_process_fn(BASIC_TRANSFORM(image)).unsqueeze(0)
                     ...
                     # reshape
                     action_predict = policy.infer(domain_name='libero-1-rgb', **inputs)    
                     action_predict = action_predict.reshape(-1, 7)[0].detach().to(torch.float32).cpu().numpy()
                     action_predict = action_predict * a_std + a_mean
