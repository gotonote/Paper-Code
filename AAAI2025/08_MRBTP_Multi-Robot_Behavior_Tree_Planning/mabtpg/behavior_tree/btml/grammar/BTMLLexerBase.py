...
class BTMLLexerBase(Lexer):
    ...
    def nextToken(self):
        ...
        if self._input.LA(1) == BTMLParser.EOF and len(self.indents) != 0:
            # Remove any trailing EOF tokens from our buffer.
            self.tokens = [token for token in self.tokens if token.type != BTMLParser.EOF]

            # First emit an extra line break that serves as the end of the statement.
            self.emitToken(self.commonToken(BTMLParser.NEWLINE, '\n'))
    ...
    def createDedent(self):
        return self.commonToken(BTMLParser.DEDENT, '')
    ...
    def onNewLine(self):
        ...
        if self.opened > 0 or (next_next != -1 and next_ in (self.DEF,)):
            self.skip()
        else:
            self.emitToken(self.commonToken(BTMLParser.NEWLINE, new_line))
            ...
            if indent == previous:
                ...
                self.emitToken(self.commonToken(BTMLParser.INDENT, spaces))
            else:
