# Adapted from https://github.com/huggingface/diffusers/blob/v0.14.0/
...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.conv_in = nn.Conv2d(self.conditioning_channels, self.block_out_channels[0], kernel_size=3, padding=1)
        ...
        self.conv_out = zero_module(
            nn.Conv2d(self.block_out_channels[-1], self.conditioning_embedding_channels, kernel_size=3, padding=1)
        )
...
class ControlNetModel(ModelMixin, ConfigMixin):
    ...
    def from_unet(
        ...
        controlnet_use_cotracker: bool = False,
    ):
        r"""
    ...
    def forward(
        ...
        # repeat and reshape inputs
        reshape_required = (sample.ndim > 4)
        if reshape_required:
            b, c, f, h, w = sample.shape
            sample = rearrange(sample, "b c f h w -> (b f) c h w")
            timestep = rearrange(timestep.unsqueeze(1).repeat(1, f), "b f -> (b f)")
