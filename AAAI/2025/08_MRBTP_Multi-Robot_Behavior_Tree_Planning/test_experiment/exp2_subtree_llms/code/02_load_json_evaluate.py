"""Table 2 evaluation pipeline: load LLM-generated subtrees, compose actions,
run MRBTP / MAOBTP (via DMR), and dump metrics."""
...
def bind_bt(bt_list):
    """Persist BTML/graphs and bind BTs back to agents."""
    # List to hold behavior trees
...
# Function to record data and compute averages
...
reflect_times = 3  # pick which llm_outputX to use; -1 means use multi_robot_subtree_ls
...
for json_id,json_data in enumerate(json_datasets[:]):
    ...
    for with_comp_action in [False,True]:
        ...
        env.with_comp_action = with_comp_action # composite action enabled -> MAOBTP inside DMR
        env.use_comp_subtask_chain = False # Whether to use task chain

        env.use_atom_subtask_chain = False # Whether to use atomic task chain
        bt_draw = verbose
        ...
        # #########################
        # Composition Action Pre-planning get BTML and PlanningAction
        composition_action = []
        comp_btml_ls=None
        ...
        if "multi_robot_subtree_ls" in json_data:
            ...
            # Adjust all 'agent-x' to 'self', combine for all agents
            if reflect_times!=-1:
            ...
            if env.with_comp_action:
                ...
                for i in range(env.num_agent):
                    ...
                    action_model[i] = sorted(action_model[i], key=lambda x: x.cost)
        ...
        for use_comp_subtask_chain in [False, True]:
            ...
            for use_atom_subtask_chain in [False, True]:
                ...
                env.use_comp_subtask_chain = use_comp_subtask_chain   # Whether to use task chain
                env.use_atom_subtask_chain = use_atom_subtask_chain # Whether to use atomic task chain
