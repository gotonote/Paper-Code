...
class Router(nn.Module):
    def __init__(self, num_choises, first=False):
        super().__init__()
        self.num_choises = num_choises
        if first:
            self.prob = torch.nn.Parameter(
                torch.tensor([5.0] * num_choises), requires_grad=False
            )
        else:
    ...
    def forward(
        self, x=None
    ):  # Any input will be ignored, only for solving the issue of https://github.com/pytorch/pytorch/issues/37814
...
class DiT(nn.Module):
    ...
    def add_router(self, num_steps, timestep_map):
        self.routers = torch.nn.ModuleList(
            [Router(2 * self.depth) for _ in range(num_steps - 1)]
            + [Router(2 * self.depth, first=True)]
        )
    ...
    def forward_(
        ...
        y = self.y_embedder(
            y, self.training, force_drop_ids=label_for_dropout
        )  # (N, D)
        c = t + y

        router_idx = self.timestep_map[timestep]
        if hasattr(self, "lambda_gen") and self.lambda_gen:
            weights = self.mask[router_idx]
            router_l1_loss = None
        else:
        ...
        for i, block in enumerate(self.blocks):
            ...
            if not fix_reuse_feature:
                if self.reuse_feature[i] is None:
                    self.reuse_feature[i] = [
                        reuse_feature[0].detach(),
                        reuse_feature[1].detach(),
                    ]
                else:
                    if weights[i * 2] > thres:
                        self.reuse_feature[i][0] = reuse_feature[0].detach()
                    if weights[i * 2 + 1] > thres:
                        self.reuse_feature[i][1] = reuse_feature[1].detach()
    ...
    def forward(
        ...
        if activate_router:
            model_out, router_l1_loss = self.forward_(
                combined,
                t,
                y,
                thres,
        ...
        if activate_router:
            return torch.cat([eps, rest], dim=1), router_l1_loss
